...
namespace HelloAR
{
	...
	public class MainActivity : AppCompatActivity, GLSurfaceView.IRenderer, Android.Views.View.IOnTouchListener
	{
		...
		GLSurfaceView mSurfaceView; // OpenGL
      
		Session mSession; //Arcore
		BackgroundRenderer mBackgroundRenderer = new BackgroundRenderer(); //Android
		GestureDetector mGestureDetector;
		Snackbar mLoadingMessageSnackbar = null;
        DisplayRotationHelper mDisplayRotationHelper;

		ObjectRenderer mVirtualObject = new ObjectRenderer();//Arcore ,obj read
        ObjectRenderer mVirtualObjectShadow = new ObjectRenderer();//Arcore


        PlaneRenderer mPlaneRenderer = new PlaneRenderer();
		PointCloudRenderer mPointCloud = new PointCloudRenderer();//PointCloud
        ...
        //          //	// Draw background.
        //          mBackgroundRenderer.Draw(frame); //카메라가 비추는 것을 BackgroundRender
        ...
        public void OnDrawFrame(IGL10 gl)
        {
            ...
            try
            {
                ...
                float scaleFactor = 3;
                mVirtualObject.updateModelMatrix(scaleFactor);

                float[] projmtx2 = new float[16];
                Android.Opengl.Matrix.SetIdentityM(projmtx2, 0);

                mVirtualObject.Draw(viewmtx, projmtx2, lightIntensity);
